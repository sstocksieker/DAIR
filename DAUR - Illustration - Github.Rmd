...
relance = T # = T to restart the simulations, F to run with the loaded workspace (v AISTATS)

# Workspace setting
setwd("C:/Users/samgo/OneDrive/Perso/Thèse/Travaux/Exogenous Sampling/Illustration")

if (relance == F){
  load("C:/Users/samgo/OneDrive/Perso/Thèse/Travaux/Exogenous Sampling/Illustration/WKS/wks-Illu-relance.RData")
}
```
...
The impacts of the imbalanced sample on the prediction are visible on the part "Performance Evaluation"
...
Graphical analysis of rebalanced sample
...
source code : https://rdrr.io/cran/ROSE/man/ROSE-package.html
...
Same approach than ROSE but with another bandwidth matrix
...
```{r smoothedbootstrap result, echo=FALSE}
ech_synth = kde_boot
name_synth = "kde_boot"
graph(ech_synth,name_synth)
```
...
if (relance == T){
  ...
  ech0_1000 = ech0*1000 # bug with [0;1]-continuous variable within the new version of gaussiancopula (v.0.5.2 vs v.0.5.1)
  ...
  ech_copule = py$ech_copule_py/1000
...
By definition, the copula simulations do not go beyond the min-max values 
...
The following chunk worked with sdv v0.5.1 and reticulate v1.25. With sdv v.0.5.2 and sdv 0.5.2, it does not work. To get a synthetic data GAN sample, we need to run the next chunk in python (that does not work in R and work in Python)
...
```{R GAN_python}
### Export dataframe for python
write.csv(ech0,"ech0.csv",row.names = F)

### Import and run this code in python environment
# import os
# import pandas as pd
# os.chdir("C:/Users/samgo/OneDrive/Perso/Thèse/Travaux/Exogenous Sampling/Illustration")
# ech0 = pd.read_csv("ech0.csv")
# from sdv.tabular import CTGAN
# ctgan = CTGAN(epochs=10000)
# ctgan.fit(ech0*1000)
# ech_ctgan_py = ctgan.sample(100000)/1000
# ech_ctgan_py.to_csv("ech_ctgan_py.csv",index=False)

### Import and WR
ech_GAN = read.csv("ech_ctgan_py.csv")
ech_GAN = WR(ech_GAN,ech_GAN$X,n_ech)
```
...
### Export dataframe for python
ech_MC = cbind(ech0[,c("X","Y")],"cluster" = DMC$classification)
write.csv(ech_MC,"ech_MC.csv",row.names = F)

### Import and run this code in python environment
# import os
# import pandas as pd
# os.chdir("C:/Users/samgo/OneDrive/Perso/Thèse/Travaux/Exogenous Sampling/Illustration")
# ech_MC = pd.read_csv("ech_MC.csv")
# from sdv.tabular import CTGAN
# from ctgan import CTGANSynthesizer
# ctgan = CTGANSynthesizer(epochs=10000)
# discrete_columns = ['cluster']
# ctgan.fit(ech_MC*1000,discrete_columns)
# ech_ctganSynth_GMM = ctgan.sample(100000)/1000
# ech_ctganSynth_GMM.to_csv("ech_ctganSynth_GMM.csv",index=False)
# ech_ctganSynth_GMM.head()

### Import and WR
ech_ctganSynth_GMM = read.csv("ech_ctganSynth_GMM.csv")
ech_ctganSynth_GMM = WR(ech_ctganSynth_GMM,ech_ctganSynth_GMM$X,n_ech)
```
...
```{r Smote_clust_graph, echo=FALSE}
