...
rerun = F # = T to restart the simulations, F to run with the loaded workspace (v AISTATS)

# Workspace setting
setwd("C:/Users/user/OneDrive/Perso/Thèse/Travaux/Exogenous Sampling/Illustration")

if (rerun == F){
  load("C:/Users/user/OneDrive/Perso/Thèse/Travaux/Exogenous Sampling/Illustration/WKS/wks-Illu-AISTATS.RData")
}
```
...
if (rerun == T){
  ...
  ech0_1000 = ech0 # bug with [0;1]-continuous variable within the new version of gaussiancopula (v.0.5.2 vs v.0.5.1)
...
points(ech0$X,ech0$Y,col="red")
```
*Error* : "Error in SMOGNRegress(X ~ ., ech0) : All the points have relevance 0. 
